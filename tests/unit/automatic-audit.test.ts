...
const USERS_TABLE = `auto_audit_users_${TIMESTAMP}`;
const VEHICLES_TABLE = `auto_audit_vehicles_${TIMESTAMP}`;
...
describe("Automatic Audit Logging (Integration)", () => {
  ...
  beforeEach(async () => {
    ...
    // Only clean audit logs for our tables
    await originalDb.execute(
  ...
  describe("UPDATE operations", () => {
    it("should automatically log updates with before/after values", async () => {
      ...
      // Note: oldValues will be null because captureOldValues defaults to false
      expect(logs[0].newValues).toMatchObject({
        name: "Updated Name",
        email: "updated@example.com",
      });
    ...
    it("should not log update if nothing changed (when captureOldValues=true)", async () => {
      // Need to create a new logger with captureOldValues enabled for this test
      const auditLogger = createAuditLogger(originalDb, {
        tables: [USERS_TABLE],
